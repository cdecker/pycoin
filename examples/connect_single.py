...
def send_version(connection, message):
    ...
    v.addr_recv = network.messages.Address(connection.host[0], True, connection.host[1], 1)
    v.best_height = 0
...
def handle_message(connection, message):
    ...
    print '%s message from %s:%d' % (message.type, connection.host[0], connection.host[1])


network_client.register_handler(network.ConnectionEstablishedEvent.type, send_version)
network_client.register_handler(network.messages.VersionPacket.type, send_verack)
